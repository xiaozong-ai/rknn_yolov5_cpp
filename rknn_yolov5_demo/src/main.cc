...
// 遍历测试集所有图像进行推理
static void traverse_test_dataset(const std::string &testdataset_dir_path, model_info m_info)
{
    DIR *dir;
    struct dirent *entry;
    ...
    while (1)
    {
        ...
        if (temp_img_name.find(".jpg") != temp_img_name.npos) {
            ...
            img_width = cvt_color_img.cols;
            img_height = cvt_color_img.rows;
        }
    }
...
int main_2(int argc, char **argv)
{
    ...
    // rknn_context ctx;
    traverse_test_dataset(test_dataset_path, m_info);
}
...
int main(int argc, char **argv)
{
    ...
    printf("input0 length = %u\n", outputs[0].size);
    printf("input1 length = %u\n", outputs[1].size);
    printf("input2 length = %u\n", outputs[2].size);

    post_process_fp32((float *)outputs[0].buf, (float *)outputs[1].buf, (float *)outputs[2].buf, height, width,
                      box_conf_threshold, nms_threshold, scale_w, scale_h, out_zps, out_scales, &detect_result_group);
